...
export function CloneDialog({ name, catalogId, projectId, url, defaultParent, onClose, onCloned, onToast }: Props) {
  ...
  async function pick() {
    try {
      ...
      onToast(error instanceof Error ? error.message : "No se eligió carpeta");
    }
  }
  ...
  async function clone() {
    ...
    try {
      ...
      onToast(error instanceof Error ? error.message : "No se pudo clonar");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="sheet-scrim fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      <div className="w-[min(32rem,92vw)] rounded-lg bg-[var(--paper)] p-5 shadow-xl" onClick={(event) => event.stopPropagation()}>
        <p className="text-[0.7rem] font-bold uppercase tracking-[0.18em] text-[var(--ink-soft)]">Descargar</p>
        ...
        <p className="mt-3 text-sm text-[var(--ink-soft)]">
          Se creará una carpeta <span className="font-mono text-xs">{name}</span> dentro del destino que elijas.
        </p>
