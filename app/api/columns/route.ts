...
export async function POST(request: Request) {
  ...
  const column = addColumn(store, body.title || "Nueva columna");
  writeStore(store);
...
export async function PATCH(request: Request) {
  ...
  if (!column) return NextResponse.json({ ok: false, error: "Columna no encontrada" }, { status: 404 });
...
export async function DELETE(request: Request) {
  ...
  if (store.columns.length <= 1) {
    return NextResponse.json({ ok: false, error: "Hace falta al menos una columna" }, { status: 400 });
  }
  const remaining = store.columns.filter((column) => column.id !== body.id);
  if (remaining.length === store.columns.length) {
    return NextResponse.json({ ok: false, error: "Columna no encontrada" }, { status: 404 });
  }
